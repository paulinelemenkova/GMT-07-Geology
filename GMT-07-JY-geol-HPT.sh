...
gmt psscale -Dg137.0/-70+w26.0c/0.4c+v+o0.3/0i+ml -Rhpt_relief.nc -J -Cmyocean.cpt \
...
gmt psbasemap -R -J \
    ...
    -B+t"Geologic settings in New Zealand, Hikurangi, Puysegur and Hjort trenches" -O -K >> $ps
# Step-10. Add projection scale
gmt psbasemap -R -J \
    --FONT=11p,Helvetica,black \
    --MAP_TITLE_OFFSET=0.3c \
    -Lx14c/-3.6c+c50+w800k+l"Mercator projection. Scale (km)"+f \
    -UBL/-5p/-102p -O -K >> $ps
# Geology
...
-F+f11p,Helvetica,black+jLB -Gwhite@10 >> $ps << EOF
182.8 -37.5 43 mm/yr
EOF

# texts
gmt pstext -R -J -N -O -K \
-F+f13p,Helvetica-Bold,black+jLB -Gwhite@20 -Wthinnest,darkbrown >> $ps << EOF
146 -42 TASMAN
146 -32.0 AUSTRALIA
...
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,Helvetica−Bold,black+jLB -Gwhite@10 >> $ps << EOF
162 -58.1 Macquarie Ridge
162 -58.7 Complex
161 -57.0 Hjort
161 -57.5 Plateau
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,Helvetica−Bold,black+jLB+a-310 -Gwhite@10 >> $ps << EOF
161.2 -53 Macquarie Arc
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,Helvetica−Bold,black+jLB+a-50 -Gwhite@10 >> $ps << EOF
162.0 -33 Lord Howe Rise
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,Helvetica,black+jLB+a-325 -Gwhite@10 >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+f14p,Helvetica-Bold,white+jLB >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,Helvetica,black+jLB+a-40 -Gwhite@10 >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,Helvetica−Bold,red+jLB+a-301 -Gwhite@10>> $ps << EOF
177.9 -42.0 Hikurangi Trench
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,Helvetica−Bold,red+jLB+a-308 -Gwhite@10>> $ps << EOF
158.5 -53.0 P u y s e g u r  T r e n c h
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,Helvetica−Bold,red+jLB+a-110 -Gwhite@10>> $ps << EOF
159 -56.0 Hjort
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,Helvetica−Bold,red+jLB+a-56 -Gwhite@10>> $ps << EOF
158.2 -58.0 Trench
EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,Helvetica−Bold,red+jLB -Gwhite@10 >> $ps << EOF
...
# GEOL texts
gmt pstext -R -J -N -O -K \
-F+f14p,Helvetica−Bold,blueviolet+jLB -Gwhite@10>> $ps << EOF
...
# Add legend
gmt pslegend -R -J -Dx0.5/-2.8+w15.7c+o0.1/0.1c \
